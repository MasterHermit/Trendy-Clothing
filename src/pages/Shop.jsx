...
export const shopLoader = async ({ request }) => {
  ...
  try {
    const response = await axios(
      `http://localhost:8000/products?${filterObj.brand === "all" ? "" : `brandName=${params.brand}`
      }&${filterObj.category === "all" ? "" : `category=${params.category}`}&${filterObj.gender === "all" ? "" : `gender=${params.gender}`
      ...
      }&${filterObj.search && `q=${filterObj.search}`}&${filterObj.price && `price.current.value_lte=${filterObj.price}`
